...
if [ -f /tmp/nvidia_vgpu_driver ]; then
  ...
  if [ $DIFF -gt $CHK_TIMEOUT ]; then
    echo -n "$(wget -qO- https://api.github.com/repos/novidio/unraid-novidio-vgpu-driver/releases/tags/${KERNEL_V} | jq -r '.assets[].name' | grep "${PACKAGE}" | grep -E -v '\.md5$' | awk -F "-" '{print $3}' | sort -V | tail -10)" > /tmp/nvidia_vgpu_driver
